...
class QuestionFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    class Params:
        ...
        short_answer = factory.Trait(
            options=[fake.sentence(nb_words=3), "", "", ""], correct_answer=0
        )
...
class ResultFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    class Params:
        ...
        failing_score = factory.Trait(
            score=factory.LazyFunction(lambda: random.randint(0, 49))
        )
        ...
        for module in modules:
            ...
            for employee in employees:
                result = ResultFactory.create(
                    session, user=employee, quiz=quiz, score=random.randint(70, 100)
                )
                results.append(result)
