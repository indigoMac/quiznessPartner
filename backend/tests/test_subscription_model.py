...
from models.subscription import Invoice, PaymentMethod, Subscription, UsageRecord
...
class TestSubscriptionModel:
    ...
    def test_create_subscription(self, db_session: Session):
        ...
        # Create a user first
        user = User(
            email="test@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_subscription_plan_types(self, db_session: Session):
        ...
        user = User(
            email="plans@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
        ...
        subscriptions = (
            db_session.query(Subscription).filter(Subscription.user_id == user.id).all()
        )
    ...
    def test_subscription_cancellation(self, db_session: Session):
        ...
        user = User(
            email="cancel@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
...
class TestPaymentMethodModel:
    ...
    def test_create_payment_method(self, db_session: Session):
        ...
        user = User(
            email="payment@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
...
class TestUsageRecordModel:
    ...
    def test_create_usage_record(self, db_session: Session):
        ...
        user = User(
            email="usage@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
...
class TestInvoiceModel:
    ...
    def test_create_invoice(self, db_session: Session):
        ...
        # Create user and subscription
        user = User(
            email="invoice@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
