...
class TestPaymentService:
    ...
    async def test_create_checkout_session(self, mock_stripe, db_session: Session):
    ...
    async def test_create_checkout_session_free_plan(self, db_session: Session):
        ...
        user = User(
            email="free_plan@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_get_usage_limits_free_user(self, db_session: Session):
        ...
        user = User(
            email="free_user@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_get_usage_limits_pro_user(self, db_session: Session):
        ...
        user = User(
            email="pro_user@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_check_quiz_limit_free_user_under_limit(self, db_session: Session):
        ...
        user = User(
            email="under_limit@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_check_quiz_limit_free_user_at_limit(
        self, mock_check_limit, db_session: Session
    ):
        """Test quiz limit check for free user at limit."""
        user = User(
            email="at_limit@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    def test_check_quiz_limit_enterprise_user(self, db_session: Session):
        ...
        user = User(
            email="enterprise@example.com", hashed_password="hashed123", is_active=True
        )
        db_session.add(user)
        db_session.commit()
    ...
    async def test_handle_webhook_successful_payment(
        self, mock_stripe, db_session: Session
    ):
