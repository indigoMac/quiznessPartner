...
class QuizNessUser(HttpUser):
    ...
    def create_quiz(self):
        ...
        with self.client.post(
            "/generate-quiz", json=quiz_data, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                quiz = response.json()
    ...
    def get_quiz(self):
        ...
        if hasattr(self, "last_quiz_id") and self.last_quiz_id:
            with self.client.get(
                f"/quiz/{self.last_quiz_id}", headers=self.headers, catch_response=True
            ) as response:
                if response.status_code == 200:
                    quiz = response.json()
            ...
            if questions:
                ...
                with self.client.post(
                    ...
                    if response.status_code == 200:
                        ...
                        response.success()
                        logger.debug(
                            f"Quiz submitted, score: {result.get('score', 0)}%"
                        )
                    else:
    ...
    def upload_document(self):
        ...
        with self.client.post(
            "/upload-document", files=files, headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
                quiz = response.json()
                self.last_quiz_id = quiz.get("id")
    ...
    def get_user_profile(self):
        ...
        with self.client.get(
            "/auth/me", headers=self.headers, catch_response=True
        ) as response:
            if response.status_code == 200:
...
class DatabaseStressUser(HttpUser):
    ...
    def create_and_submit_quiz(self):
        ...
        response = self.client.post(
            "/generate-quiz", json=quiz_data, headers=self.headers
        )
        if response.status_code == 200:
            quiz = response.json()
            quiz_id = quiz["id"]
            ...
            if response.status_code == 200:
                ...
                self.client.post(
                    "/submit-answers", json=submission, headers=self.headers
                )
